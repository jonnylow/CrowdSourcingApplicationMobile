                  ...
                  if(data.status[0]=="do not exist")
                  {
                    ...
                    $http.get(urlString)
                      .success(function (data) {
                        ...
                        var sendEmail = apiUrl+"email/sendEmail.php?email=jonathanlow.2013@sis.smu.edu.sg&title=[CareRide Alert] New Applicant on CareRide&message=There is a new transport application from "+window.localStorage.getItem("loginUserName") ;
                        $http.get(sendEmail)
                          .success(function (data) {
                          //email send
                          })
